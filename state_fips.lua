...
function M.stateFips()
  ...
  for _ = 1, count do
    ...
    for s, _ in string.gmatch(text, "(%w+%.?%s?(%w*%s*)(%w*%s*)(%w*%s*)(%w*%s*))(,?%s*)") do
      for i, _ in ipairs(states) do
        if vim.trim(s) == states[i]["name"]:upper() or vim.trim(s) == states[i]["abb"] then
          local code = states[i]["fips"]
          text = vim.fn.substitute(text, s, code, "")
        end
      end
    end
