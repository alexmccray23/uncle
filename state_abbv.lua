...
function M.stateAbbv()
  ...
  for _ = 1, count do
    ...
    local substring = text:sub(40)
    local state = vim.fn.trim(substring)
    for i, _ in ipairs(states) do
      if state == states[i]['name']:upper() then
        local abbv = states[i]['abb']
        text = text:gsub(state, abbv)

        vim.api.nvim_buf_set_lines(0, current_line - 1, current_line, false, { text })
...
vim.api.nvim_create_user_command("StateAbbv", M.stateAbbv, {})
