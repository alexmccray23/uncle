...
-- Neovim plugin for adding mean and median values in Uncle
...
function M.replaceColumns()
  ...
  for i, k in ipairs(spec_table) do
    if not k['question']:match("^[oO][rR]$") then
      ...
      if wfield == 1 then
        if scol == ecol then
          syntax = "A(" .. scol .. "), 1!" .. scol .. "-1:9;FDP 1"
          fullSpec = vim.fn.substitute(fullSpec, spec, syntax, '')
        else
          syntax = "A(" .. scol .. "+" .. ecol .. "), 1!" .. scol .. "+" .. ecol .. "-1:9;FDP 1"
          fullSpec = vim.fn.substitute(fullSpec, spec, syntax, '')
        end
      else
        if nfield == 1 then
          syntax = "A(" .. scol .. ":" .. ecol .. "," .. code .. "), R(1!" .. scol .. ":" .. ecol .. "," .. code .. ")"
          fullSpec = vim.fn.substitute(fullSpec, spec, syntax, '')
...
vim.api.nvim_create_user_command("Mean", M.mean, {})
vim.api.nvim_create_user_command("Median", M.median, {})
